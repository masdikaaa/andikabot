...
const messageTemplates = {
    ...
    success: (text, imageUrl) => ({
        image: { url: imageUrl },
        caption: "DIBUAT OLEH Andika Bot",
        contextInfo: channelInfo
    })
};
    ...
    try {
        if (!q) {
            return await sock.sendMessage(
                chatId,
                messageTemplates.error("⚠️ Tolong berikan teks yang akan dibuat.\nContoh: *.metallic Nick*")
            );
        }
        ...
        try {
            ...
            switch (type) {
                ...
                default:
                    return await sock.sendMessage(
                        chatId,
                        messageTemplates.error("❌ Tipe generator tidak dikenal.")
                    );
            }

            if (!result || !result.image) {
                throw new Error('URL gambar tidak diterima dari API');
            }
            ...
            await sock.sendMessage(
                chatId,
                messageTemplates.error(`❌ Terjadi kesalahan: ${error.message}`)
            );
        }
    } catch (error) {
        ...
        await sock.sendMessage(
            chatId,
            messageTemplates.error("❌ Terjadi kesalahan. Coba lagi nanti.")
        );
    }
}
